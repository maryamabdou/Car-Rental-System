...
if (isset($_POST['Display3'])) {
    ...
    $sql = "(SELECT plate_id, model, year, car_status FROM car) 
    UNION 
    (SELECT plate_id, model, year, 'rented' AS car_status FROM car 
    NATURAL JOIN `return` 
    NATURAL JOIN reserve WHERE '$date' >= start_date AND '$date' <= return_date)";
